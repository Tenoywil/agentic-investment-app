...
async function main(): Promise<void> {
  ...
  try {
    ...
    await db
      .insert(limits)
      .values({ userId: demoId })
      .onConflictDoNothing({ target: limits.userId });

    // Reset-then-insert the demo user's owned rows ---------------------------
    await db.delete(holdings).where(eq(holdings.userId, demoId));
    await db.delete(connectedAccounts).where(eq(connectedAccounts.userId, demoId));
    await db.delete(goals).where(eq(goals.userId, demoId));
    await db.delete(approvals).where(eq(approvals.userId, demoId));
    await db.delete(riskProfiles).where(eq(riskProfiles.userId, demoId));
    await db.delete(agentMessages).where(eq(agentMessages.userId, demoId));
    ...
    const accounts = [
      ...
      },
      {
        ...
        holdings: [
          { name: 'Sagicor Sigma Global Fund', value: 8200, ret: '+4.1%', slug: 'sagrex' },
        ],
      },
      {
        code: 'PRV',
    ...
    for (const acc of accounts) {
      ...
      const [account] = await db
        ...
        .values({ userId: demoId, partnerId, label: acc.label })
    ...
    await db.insert(agentMessages).values([
      ...
      { userId: demoId, role: 'user', content: 'What about the idle cash?' },
      {
        userId: demoId,
        role: 'agent',
        content:
