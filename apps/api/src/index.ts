...
const server = Bun.serve<WsData>({
  ...
  async fetch(req, srv) {
    ...
    // forwards it and the session cookie stays first-party.
    if (url.pathname === '/api/sse') {
        ...
        start(controller) {
          const write = (chunk: string) => {
            try {
              ...
              // Dropping the frame is correct; throwing would take down the
          ...
          // between here and the browser, and both Render and Vercel will close
          // it. A comment frame is ignored by EventSource and keeps it open.
          write(': connected\n\n');
          // The hub keeps no replay buffer, so a client that reconnects has a
          // hole in its history. Reconnecting quickly keeps the hole small; the
          // client closes it by refetching on every open (lib/realtime.ts).
          write('retry: 3000\n\n');
      ...
      return new Response(stream, {
        headers: {
          ...
          // Nginx and several CDNs buffer a response until it completes, which
          // for a stream that never completes means delivering nothing at all.
          'x-accel-buffering': 'no',
          // This route is served by Bun directly, ahead of the Hono app, so the
          // CORS middleware never sees it. In production that is invisible —
          // the browser reaches it same-origin through the web app's rewrite —
          // but in local development the web app is on :3000 and the API on
          // :3001, and without these the stream is refused before it opens.
